...
def train_model(base_model_fn, model_name):
    ...
    # ── FIXED: Save as .keras format instead of .h5 ──────────
    callbacks = [
        keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(RESULTS_DIR, f"{model_name}_best.keras"),
    ...
    training_time = round(elapsed / 60, 2)
    ...
    # ── FIXED: Save final model as .keras format ───────────────
    model.save(os.path.join(RESULTS_DIR, f"{model_name}_final.keras"))
    print(f" Model saved to: {RESULTS_DIR}/{model_name}_final.keras")
...
if __name__ == "__main__":
    ...
    # ── FIXED: Train only MobileNetV3 (others already done) ───
    models_to_train = [
        (MobileNetV3Large, "MobileNetV3"),
    ]

    # ── To retrain ALL models, replace above with this: ───────
    # models_to_train = [
    #     (ResNet50,         "ResNet50"),
    ...
    # ── Load existing results to merge into summary ────────────
    for name in ["ResNet50", "DenseNet121", "MobileNetV3"]:
        if name not in all_results:
            json_path = os.path.join(RESULTS_DIR, f"{name}_results.json")
    ...
    print(f"\n Now run: python step5_visualize.py")